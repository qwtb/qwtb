...
%
% MH Oct2010
% v2.23.1 errorbars in plots changed to confidence intervals to decrease cluttered graphs by M>
% v2.22.1 version for GNU Octave by M>
...
if isfield(data,'rate') && data.rate > 0 % if data rate was given
    ...
    % fix Martin Šíra 14/04 - did not worked for small periods, compare 2 examples:
    %           tmstep = 1e-6, length(data.freq) = 20, halftime would be 0,
    %           tmstep = 0.1, length(data.freq) = 20, halftime would be 1,
    % original: halftime = round(tmstep*length(data.freq)/2);
    % fixed: 
    halftime = tmstep*length(data.freq)/2;
...
if verbose >= 2 % show all data
    ...
    % quick fix to run in GNU Octave
    % catch ME
    catch
        figure;
        ...
        % quick fix to run in GNU Octave - just commented line
    ...
    legend({'data (centered on median)','5x MAD outliers',['Linear Fit (' num2str(s.linear(1),'%g') ')']});
    % tighten up
...
if verbose >= 1 % show ADEV results
    ...
    if ~isempty(sm)
        ...
        % error bars were over mean line, only confidence intervals now shown. by M>
        %hold on; plot([tau; tau],[sm+sme; sm-sme],'-k','LineWidth',max(plotlinewidth-1,2));
        hold on; plot(tau,sm+sme,'-k','LineWidth',max(plotlinewidth-1,2));
                 plot(tau,sm-sme,'-k','LineWidth',max(plotlinewidth-1,2));
